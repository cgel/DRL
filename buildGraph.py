...
def createQNetwork(input_state, action, action_num, summaryCollection):
    ...
    hidden_state = tf.reshape(nn_head, [-1, h_conv3_shape[1] * h_conv3_shape[2] * h_conv3_shape[3]], name="conv3_flat")
    ...
    predicted_next_hidden_state = tf.concat(1, [action_one_hot, hidden_state], name="one_hot_concat_state")
    predicted_next_hidden_state = add_linear_layer(predicted_next_hidden_state, size=512, _summaryCollection=summaryCollection+"_prediction", layer_name="predicted_next_hidden_state_linear_1")
    predicted_next_hidden_state = add_linear_layer(predicted_next_hidden_state, size=hidden_state_shape[1], _summaryCollection=summaryCollection+"_prediction", layer_name="predicted_next_hidden_state_linear_2")

    tf.get_variable_scope().reuse_variables()
    predicted_next_Q = hidden_state_to_Q(predicted_next_hidden_state, "future_Q", summaryCollection+"_prediction")
...
def build_train_op(Q, Y, action, predicted_next_Q, next_Y, action_num, lr, alpha=1):
    with tf.name_scope("loss"):
        action_one_hot = tf.one_hot(action, action_num, 1., 0., name='action_one_hot')
        DQN_acted = tf.reduce_sum(Q * action_one_hot, reduction_indices=1, name='DQN_acted')

        loss = tf.reduce_mean(clipped_l2(Y, DQN_acted))
        future_loss = tf.reduce_sum(clipped_l2(predicted_next_Q, next_Y), name="future_loss")
        ...
        tf.add_to_collection("DQN_summaries", tf.scalar_summary("max_predicted_next_Q_0", max_predicted_next_Q[0]))
